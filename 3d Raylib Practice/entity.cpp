...
Entity::Entity(const char* modelpath, const char* texturepath, std::string inName, Vector3 inPos)								//Contructor [modelpath, texturepath, name, position]
{
	...
	entityCamera.position = { entityPosition.x + 10.0f, entityPosition.y + 10.0f, entityPosition.z + 10.0f};					//Calculating camera position with the gap, anyway this parameter will be instantly rewritten by cameraMovementThridPerson
